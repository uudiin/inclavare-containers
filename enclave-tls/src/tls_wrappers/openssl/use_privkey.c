...
tls_wrapper_err_t openssl_use_privkey(tls_wrapper_ctx_t *ctx, void *privkey_buf, size_t privkey_len)
{
	...
	int ret = SSL_CTX_use_PrivateKey_ASN1(EVP_PKEY_RSA, ssl_ctx->sctx, privkey_buf, (long)privkey_len);
	if (ret != SSL_SUCCESS) {
